...
function draw(t: number)
{
	...
	if (t - lastFirework > 500 && fireworks.length < 10)
	{
		lastFirework = t;
		fireworks.push(new Firewok().start(
			{ x: Lib.randomInt(0, canvas.width), y: canvas.height },
			{ x: Lib.randomInt(0, canvas.width), y: Lib.randomInt(0, canvas.height * 3 / 4) })
		);
	}
