...
export function createParticle(p) {
    return {
        ...
        c: typeof p.c == "string" ? p.c : hslColor(typeof p.c[0] == "number" ? p.c[0] : randomNum(...p.c[0]), typeof p.c[1] == "number" ? p.c[1] : randomNum(...p.c[1]), typeof p.c[2] == "number" ? p.c[2] : randomNum(...p.c[2])),
...
export function calcParticle(particle, dt) {
    particle.x += particle.dx * dt;
    particle.y += particle.dy * dt;
    particle.r += particle.dr * dt;
    particle.a += particle.da * dt;
}
export function loopParticle(particle, w, h) {
    let loopX = 0;
    let loopY = 0;
